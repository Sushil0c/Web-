...
+ Fixed situation when Sort button was invisible when one of Folder View modes is enabled.

+ Added "Show chapters" to the Settings--Full version settings.
If by some reason you don't want to see chapters - you can disable them now.

+ On some phones running Android 13 or later, the app could not display notifications and the user could not enable notifications.
Now, if the app is not showing notifications, you can enable them in:
Android Settings--Apps--Smart AudioBook Player--Notifications.",
        link: "https://www.mediafire.com/file/cxfl1hxd2h18mox/Smart+AudioBook+Player-v10.7.7(Premium).apk"
    },
    {
        title: "Smart Tools_v20.9",
      description: ",Smart Tools - All In One is a useful kit of tools with 40+ carpenter construction measure and other tools and utilities. Use device's in-built sensors in all in one tool box app useful like a swiss army knife.

Measure tools kit:
dB level: Measure the sound dB level and its spectrum;
Location (map) with altimeter;
Distance meter;
Stopwatch;
Thermometer;
Magnetic field meter (metal detector);
Vibration level meter;
Luminosity (LUX) level meter;
Color sensor;
Speedometer;
Compass;
Battery tester;
...
Other useful utilities kit:
Unit currency and size converter;
Calculator;
...
",
     ...
     // Add more app data objects here if needed
];